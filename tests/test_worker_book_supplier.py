...
def _create_and_pay():
    ...
    res = client.post("/reservations", json=create_payload, headers={"Idempotency-Key": "wk1"})
    reservation_code = res.json()["reservation_code"]
    client.post(
        f"/reservations/{reservation_code}/pay",
        json={"payment_method_id": "pm_123"},
        headers={"Idempotency-Key": "wk1-pay"},
    )
    return reservation_code


def test_worker_confirms_reservation_and_sets_supplier_code():
    reservation_code = _create_and_pay()
    res = client.post(
        f"/workers/outbox/book-supplier/{reservation_code}",
        params={"idem_key": "sup-1"},
    )
    ...
    # Validate repo state
    stored = reservations_router._reservation_repo.reservations[reservation_code]
